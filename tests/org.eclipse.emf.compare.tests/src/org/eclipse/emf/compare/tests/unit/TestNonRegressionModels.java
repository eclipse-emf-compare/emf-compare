...
public class TestNonRegressionModels extends EMFCompareTestCase {
	...
	private void compareSnapshots(File directory) throws IOException, InterruptedException {
		if (FileUtils.listDirectories(directory).length != 0) {
			...
			final File expectedDir = new File(
					directory.getPath().replace(File.separator + "inputs", File.separator + "expected" + File.separator + "inputs"));
			...
			if (inputModels.size() == 2 && expectedSnapshot.size() == 1) {
				final MatchModel inputMatch = new MatchService().doMatch(
						inputModels.get(0), inputModels.get(1), new NullProgressMonitor());
				...
				assertEquals(testedDir + ", match doesn't match", suppressPathReferences(currentMatch), suppressPathReferences(expectedMatch));
				assertEquals(testedDir + ", diff doesn't match", suppressPathReferences(currentDiff), suppressPathReferences(expectedDiff));
			}
		}
	}
	 ...
	 * model for all references to "a href='path#element'" and supresses the path.
	 * 
	 * @param aFile
	 * 			The file to parse.
	 */
