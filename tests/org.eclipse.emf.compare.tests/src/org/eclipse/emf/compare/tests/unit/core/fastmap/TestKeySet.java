...
/**
 * Tests the behavior of the {@link FastMap}'s key set and its iterator so that they behave the same as their
...
public class TestKeySet extends TestCase {
	...
	/**
	 * Tests the {@link FastMap}'s key set. The set is expected to support the clear() operation and it
	...
	/**
	 * Tests the {@link FastMap}'s key set. The set's iterator is expected to throw a
	...
	/**
	 * Tests the {@link FastMap}'s key set. The set is expected not to support the add() operation.
	 */
	...
	/**
	 * Tests the {@link FastMap}'s key set. The set is expected to support the removeAll(Collection)
	...
	public void testRemoveAll() {
		...
		for (int i = 0; i < KEY_SET.length; i++) {
			...
			for (int j = 0; j < array.length; j++) {
				...
				assertEquals("Unexpected size of the map after removeAll() execution.", --currentMapSize, testedMap.size());
			}
		}
	}

	/**
	 * Tests the {@link FastMap}'s key set's iterator. The iterator is expected to support the
	...
	public void testRemoveViaIterator() {
		...
		for (final Iterator keyIterator = testedMap.keySet().iterator(); keyIterator.hasNext(); ) {
			...
			assertEquals("Unexpected size of the map after removal of an element.", --currentMapSize, testedMap.size());
		}
	}

	/**
	 * Tests the {@link FastMap}'s key set. The set is expected to support the remove(Object) operation and
	...
	public void testRemoveViaSet() {
		...
		for (Object key : array) {
			...
			assertEquals("Map size didn't decrease by one after removal of one element.", --currentMapSize, testedMap.size());
		}
	}

	/**
	 * Tests the {@link FastMap}'s key set. The set is expected to support the retainAll(Collection)
	...
	public void testRetainAll() {
		...
		for (int i = 0; i < KEY_SET.length; i++) {
			...
			for (int j = 0; j < array.length; j++) {
				...
				assertEquals("Unexpected size of the map after retainAll() execution.", currentMapSize--, array.length - j);
			}
		}
	}

	/**
	 * Tests the creation of the {@link FastMap}'s key set.
	 * <p>
	 * <ul>
	 * Assertions :
	...
	public void testSetCreation() {
		...
		assertEquals("Created set hasn't got the same size as its original map.", testedMap.size(), keySet.size());
