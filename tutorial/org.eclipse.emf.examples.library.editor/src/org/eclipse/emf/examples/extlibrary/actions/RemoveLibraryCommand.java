...
/**
 * The extended library example supports multi-rooted resources and therefore this command will remove a
 * library from the resource..
 */
public class RemoveLibraryCommand extends RemoveCommand implements CommandActionDelegate {
	...
	public RemoveLibraryCommand(EditingDomain domain, EList<?> list, Collection<?> collection) {
		...
		if (domain instanceof AdapterFactoryEditingDomain) {
			labelProvider = (IItemLabelProvider)((AdapterFactoryEditingDomain)domain).getAdapterFactory()
					.adapt(collection.toArray()[0], IItemLabelProvider.class);
		}
