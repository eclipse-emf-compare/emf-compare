...
/**
 * This Match engine is specific for {@link AddressbookPackage} models. It match
 * elements from a given model to another similar model and may then be used to
 * produce a {@link DiffModel}.
 * 
 * People are matched only using their name similarity, contacts are matched
 * using the enclosing people and anything else is matched re-using the generic
 * EMF Compare match engine ( {@link GenericMatchEngine} ) .
 * 
...
public class AddressBookMatcher extends GenericMatchEngine implements IMatchEngine {
    ...
    protected boolean isSimilar(EObject obj1, EObject obj2) throws FactoryException {
        /*
         * If the instance metaclasses are not the same, then we don't want to
         * match them.
         */
        ...
        /*
         * A Contact doesn't have interesting information, but contacts should
         * be matched thanks to the people containing them.
         * 
         * FIXME : many contacts inside the same people may be badly matched and
         * then differences may be detected.
         */
