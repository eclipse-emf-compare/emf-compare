...
public class MergeChangesCompare {
	 ...
	 * @param change2
	 *            Another change.
	 * @return <code>true</code>, if these two changes can be generalized or <code>false</code>, if they cannot.
	 */
	static boolean generalizable(IndepChange change1, IndepChange change2) {
		...
		if (!areGeneralizableSymbolicReferences(change1.getCorrespondingElement(), change2.getCorrespondingElement()))
			return false;
	...
	private static boolean generzlizableAddRemElementChanges(IndepAddRemElementChange change1,
		...
		if (!areGeneralizableSymbolicReferences(change1.getSubModelReference(), change2.getSubModelReference()))
			return false;
	 ...
	 * @return <code>true</code>, if thez have the same cross-references (thus, can be merged). <code>false</code>
	 *         otherwise.
	 */
	private static boolean sameCrossReferences(EObject obj1, EObject obj2, EObject root) {
		...
		/*
		 * FIXME: At the moment, I cannot see how multiple changes can be merged if they have cross-references. The
		 * problem is that the powerset will be created during change application, and, thus, the semantics completely
		 * change!!! So for now, we don't allow any merges if there are cross-references!
		 */
	...
	/**
	 * Check whether the cross references in refs are internal only, i.e. they point only to elements in flat. This
	 * ignores any dependency references.
	 * 
	 * @param refs
	 *            A mapping of cross references.
	 * @param flat
	 *            A flat list of contents elements.
	 * @return <code>true</code> if the only cross references are internal. <code>false</code> if there exists any
	 *         cross-reference that refers to an elements not in flat.
	 */
	private static boolean onlyDependencyReferences(Map<EObject, Collection<Setting>> refs, List<? extends EObject> flat) {
		for (EObject key : refs.keySet()) {
			...
			for (Setting setting : settings) {
				if (MPatchPackage.Literals.INDEP_CHANGE__DEPENDANTS.equals(setting.getEStructuralFeature())
						|| MPatchPackage.Literals.INDEP_CHANGE__DEPENDS_ON.equals(setting.getEStructuralFeature()))
					continue;
	 ...
	 * @deprecated This method is *probably* not used anymore. Having exactly the same cross-references is very
	 *             unlikely! And even if they have different cross-references, they can still be merged. Just during
	 *             difference application, they need to be resolved properly by the user. So there shouldn't be a
	 *             problem.
	 */
			...
			if (refs2.keySet().contains(key2)) {
				...
				outer2: for (Setting setting1 : settings1) {
					for (Setting setting2 : settings2) {
						...
						// unfortunately, the cross referencer also finds internal references! ignore them here!
	...
	private static boolean generzlizableMoveElementChanges(IndepMoveElementChange change1,
		...
		if (change1.getOldParent() == null || change1.getNewParent() == null || change2.getOldParent() == null
				|| change2.getNewParent() == null)
			return false;
	...
	private static boolean generzlizableUpdateReferenceChanges(IndepUpdateReferenceChange change1,
		...
		if (change1.getOldReference() != null && !change1.getOldReference().resolvesEqual(change2.getOldReference()))
			return false;
		if (change1.getNewReference() != null && !change1.getNewReference().resolvesEqual(change2.getNewReference()))
			return false;
	 ...
	 * They must be an instance of {@link ElementSetReference} with {@link OclCondition}s and their types and contexts
	 * must be equal.
	 * 
	 * @param symref1
	...
	private static boolean areGeneralizableSymbolicReferences(IElementReference symref1, IElementReference symref2) {
		...
		/*
		 * At the moment, we only allow symbolic references with ONE condition to be generalized!!!
		 * 
		 * In the future, more complex condition are imaginable like composed conditions to realize OR and AND. However,
		 * in order to merge conditions (the only generalization implemented at the moment), we must ensure that there
