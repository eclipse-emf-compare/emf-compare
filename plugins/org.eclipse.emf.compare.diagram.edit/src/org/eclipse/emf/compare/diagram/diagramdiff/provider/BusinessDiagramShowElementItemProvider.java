...
/**
 * Extension of DiagramShowElementItemProvider.
 * 
 * @author Cedric Notot <a href="mailto:cedric.notot@obeo.fr">cedric.notot@obeo.fr</a>
 */
public class BusinessDiagramShowElementItemProvider extends DiagramShowElementItemProvider {
	/**
	 * Constructor.
	 * 
	 ...
	 *            The adapter factory.
	 */
	public BusinessDiagramShowElementItemProvider(AdapterFactory pAdapterFactory) {
	...
	private String buildMessage(BusinessDiagramShowHideElement diff, String label) {
		...
		if (diff.isRemote()) {
			message = DiagramCompareUIMessages.getString(
					"DiagramCompareDifferences.showDescription.remote", label); //$NON-NLS-1$
		} else {
			message = DiagramCompareUIMessages.getString("DiagramCompareDifferences.showDescription", label); //$NON-NLS-1$
		}
		return message;
	}
}
