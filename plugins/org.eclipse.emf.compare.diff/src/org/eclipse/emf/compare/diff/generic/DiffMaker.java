...
public class DiffMaker implements DiffEngine {
	 ...
	 * <p>
	 * An attribute update is considered &quot;conflictual&quot; if it isn't multi-valued and its left (latest from head) value differs from the right
	 * (working copy) value.
	 * </p>
	 * 
	 * @param root
		...
		if (!attribute.isMany()) {
			...
			for (Object aValue : (List)rightValue) {
				...
				if (!((List)leftValue).contains(aValue) && !((List)ancestorValue).contains(aValue)) {
					...
					final RemoteRemoveAttribute operation = DiffFactory.eINSTANCE.createRemoteRemoveAttribute();
					operation.setAttribute(attribute);
					operation.setRightElement(mapping.getRightElement());
					operation.setLeftElement(mapping.getLeftElement());
					operation.setRightTarget((EObject)aValue);
	 ...
	 * @return {@link List} of all the references that have been added in the right element since the left element.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * This will create and populate a {@link List} with all the references from the <code>leftReferences</code> {@link List} that cannot be matched
	 * in the <code>rightReferences</code> {@link List}.
	 * 
	 * @param leftReferences
	 *            List of the left element reference values.
	 * @param rightReferences
	 *            List of the right element reference values.
	 * @return {@link List} of all the references that have been deleted from the right element since the left element.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * This will create the {@link ConflictingDiffGroup} and its children for a conflictual {@link AttributeChange}.
	 * 
	 * @param root
	 *            {@link DiffGroup root} of the {@link DiffElement} to create.
	 * @param attribute
	...
	/**
	 * This will create the {@link ConflictingDiffGroup} and its children for a conflictual {@link ReferenceChange}.
	 * 
	 * @param root
	 *            {@link DiffGroup Root} of the {@link DiffElement} to create.
	 * @param reference
	 ...
	 * @param mapping
	 *            Contains informations about the left, right and origin element where the given reference has changed.
	 * @throws FactoryException
	...
	/**
	 * Creates the {@link DiffGroup} corresponding to a reference's value addition under the given {@link DiffGroup}.<br/>The parameters include the
	 * list of added references which can be computed using {@link #computeAddedReferences(List, List)}.
	 * 
	 * @param root
	 ...
	 *            {@link List} of reference values that have been added in the <code>right</code> element since the <code>left</code> element.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Creates and add the {@link DiffGroup} corresponding to an {@link AttributeChange} operation to the given {@link DiffGroup root}.
	 * 
	 * @param root
	 *            {@link DiffGroup root} of the {@link DiffElement} to create.
	 * @param attribute
	...
	/**
	 * This will check the given <code>reference</code> for modification between <code>leftElement</code> and <code>rightElement</code> and
	 * create the corresponding {@link DiffElement}s under the given {@link DiffGroup}.
	 * 
	 * @param root
	 *            {@link DiffGroup Root} of the {@link DiffElement}s to create.
	 * @param reference
	 ...
	 * @throws FactoryException
	 *             Thrown if we cannot fetch <code>reference</code>'s values for either the left or the right element.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * This will check for remote {@link ReferenceChange} operations and create the corresponding {@link DiffElement}s.<br/>
	 * <p>
	 * A reference is considered &quot;remotely changed&quot; if its values differ between the left (latest from HEAD) and origin (common ancestor)
	 * model, but its values haven't changed between the right (working copy) and the origin model.
	 * </p>
	 * 
	 * @param root
	 *            {@link DiffGroup Root} of the {@link DiffElement}s to create.
	 ...
	 *            {@link List} of reference values that have been removed from the left model since the origin.
	 */
	@SuppressWarnings("unchecked")
	private void createRemoteReferencesUpdate(DiffGroup root, EReference reference, Match3Element mapping, List<EObject> remotelyAdded, List<EObject> remotelyDeleted) {
		if (!reference.isMany() && remotelyAdded.size() > 0 && remotelyDeleted.size() > 0) {
			...
			for (final Iterator<EObject> addedReferenceIterator = remotelyAdded.iterator(); addedReferenceIterator.hasNext(); ) {
				final EObject eobj = addedReferenceIterator.next();
				final RemoteAddReferenceValue addOperation = DiffFactory.eINSTANCE.createRemoteAddReferenceValue();
			...
			for (final Iterator<EObject> deletedReferenceIterator = remotelyDeleted.iterator(); deletedReferenceIterator.hasNext(); ) {
				...
				final RemoteRemoveReferenceValue delOperation = DiffFactory.eINSTANCE.createRemoteRemoveReferenceValue();
	...
	/**
	 * Creates the {@link DiffGroup} corresponding to a reference's value removal under the given {@link DiffGroup}.<br/>The parameters include the
	 * list of removed references which can be computed using {@link #computeDeletedReferences(List, List)}.
	 * 
	 * @param root
	 *            {@link DiffGroup root} of the {@link DiffElement}s to create.
	 * @param left
	 ...
	 *            {@link List} of reference values that have been removed in the <code>right</code> element since the <code>left</code> element.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Creates the {@link DiffElement} corresponding to an unique reference's value update.<br/>The parameters include the lists of added and removed
	 * references, these can be computed using {@link #computeAddedReferences(List, List)} and {@link #computeDeletedReferences(List, List)}.
	 * 
	 * @param left
	 *            Left element of the reference change.
	 * @param right
	...
	/**
	 * Checks a given {@link EReference reference} for changes related to a given <code>mapping</code> and populates the given {@link List}s with
	 * the reference values belonging to them.
	 * 
	 * @param mapping
	 *            Contains informations about the left, right and origin elements.<br/>
	 *            <ul>
	 *            <li>&quot;Added&quot; values are the values that have been added in the right element since the origin and that haven't been added
	 *            in the left element.</li>
	 *            <li>&quot;Deleted&quot; values are the values that have been removed from the right element since the origin but are still present
	 *            in the left element.</li>
	 *            <li>&quot;Remotely added&quot; values are the values that have been added in the left element since the origin but haven't been
	 *            added in the right element.</li>
	 *            <li>&quot;Remotely deleted&quot; values are the values that have been removed from the left element since the origin but are still
	 *            present in the right element.</li>
	 *            </ul>
	 * @param reference
	 *            {@link EReference} we're checking for changes.
	 * @param addedReferences
	 *            {@link List} that will be populated with the values that have been added in the right element since the origin.
	 * @param deletedReferences
	 *            {@link List} that will be populated with the values that have been removed from the right element since the origin.
	 * @param remoteAddedReferences
	 *            {@link List} that will be populated with the values that have been added in the left element since the origin.
	 * @param remoteDeletedReferences
	 *            {@link List} that will be populated with the values that have been removed from the left element since the origin.
	 * @throws FactoryException
	 *             Thrown if we cannot fetch the reference's values in either the left, right or origin element.
	 */
