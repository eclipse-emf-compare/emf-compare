...
public class GitLogicalMergeTest extends CompareGitTestCase {
	 ...
	 * </ul>
	 * <p>
	 * This is both a textual and a logical conflict. We expect both kind of merges to mark the file as conflicting,
	 * but textual merging should corrupt the model with its conflict markers whereas logical merge will leave it
	 * untouched.
	 * </p>
	 */
	 ...
	 * </ul>
	 * <p>
	 * This is a pseudo conflict at both logical and textual level, which can thus be automatically merged in both cases.
	 * </p>
	 * @throws Exception
	 */
	...
	/**
	 * Sets up the repository so that there are three commits affecting two files with no links between them (not part
	 * of a single logical model).
	 * <ul>
	 * <li>initial commit : "file1" contains package P1 and classes C1 and C2. "file2" contains P2 containing C3 and C4.</li>
	 * <li>master commit : delete C2</li>
	 ...
	 * </ul>
	 * <p>
	 * This is a textual conflict in both files, but there are no logical conflicts. Textual merge will corrupt both
	 * models on top of being unable to finish the merge, logical merge will end successfully.
	 * </p>
	 * @throws Exception
	 */
	...
	/**
	 * Sets up the repository so that there are three commits affecting two interlinked files (part of the same logical
	 * model). The files won't be part of the same logical model initially, the link will be created in one of the
	 * commits we'll merge.
	 * <ul>
	 * <li>initial commit : "file1" contains package P1 and classes C1 and C2. "file2" contains P2 containing C3 and C4.</li>
	 ...
	 * </ul>
	 * <p>
	 * There is no textual conflict in these changes, so a textual merge will end successfully, corrupting the model
	 * by breaking the links between them. However, logical merging will see the conflict and should mark both files
	 * as conflicting, without touching either of them.
	 * </p>
	 * <p>
	 * This case also presents the particularity of having a "single file" locally (file1 is not linked to file2) whereas
	 * the model is comprised of two files in the right. This will allow us to make sure that we are able to resolve
	 * the logical model in such cases.
	 * </p>
	 * @throws Exception
	 */
	private void setUp007() throws Exception {
		...
		/* The files are unrelated locally, but they are part of the same logical model on the remote. We expect EMF
		 * Compare to detect that when starting from file1 (as the remote file1 references the remote file2), but not
		 * when starting from file2 (since in such a case, we have no way to discover the cross reference). */
		compareBothDirectionsAndCheck(iFile1, MASTER, BRANCH, 1, 1, 1);
		compareBothDirectionsAndCheck(iFile2, MASTER, BRANCH, 0, 1, 0);
	}
	
	/**
	 * See {@link #setUp007()} for the description of this use case, except that we reverse the changes made to file1
	 * and file2 so that the "leaf" of the logical model is found before its root in the tree, making sure that this
	 * case will not fail to merge properly.
	 * 
	 * @see #setUp007()
	 * @throws Exception
	 */
	private void setUp007_reverse() throws Exception {
		...
		/* The files are unrelated locally, but they are part of the same logical model on the remote. We expect EMF
		 * Compare to detect that when starting from file2 (as the remote file2 references the remote file1), but
		 * not when starting from file1 (since in such a case, we have no way to discover the cross reference). */
	...
	/**
	 * Sets up the repository so that there are three commits affecting two interlinked files (part of the same logical
	 * model). The files will be linked together as a single logical model in all three sides of the comparison.
	 * <ul>
	 * <li>initial commit : "file1" contains package P1 and classes C1 and C2. "file2" contains P2 containing C3 and C4.
	 * C3 is a super-type of C1, linking the two files together.</li>
	 * <li>master commit : renamed C1 into C10</li>
	 * <li>branch commit : replace C3 by C4 as a super-class of C1, rename C4 into C40</li>
	 * </ul>
	 * <p>
	 * There is a textual conflict in the first file, none in the second. We expect the textual merge to corrupt the
	 * model with its textual conflict markers. There are no logical conflicts, so logical merging should end
	 * successfully.
	 * </p>
	 * @throws Exception
	 */
	private void setUp008() throws Exception {
		...
		/* The files are related locally, but EMF Compare cannot detect that when starting from file2 (it is file1
		 * that references file2) since the three sides of this comparison are created using the subscriber's data. */
	...
	/* 
	 * Try and merge textually. This has no relation with EMF Compare, but it does check that :
	 * A - the models were properly created by setUp001, and
	 * B - the textual merge ends up corrupting the models.
	 */
	@Test
	...
	/*
	 * Try and merge logically. There is a conflict, but contrarily to merge001_textual,
	 * we expect the model not to be corrupted by this attempt.
	 */
	@Test
	...
	/* 
	 * Try and merge textually. This has no relation with EMF Compare, but it does check that :
	 * A - the models were properly created by setUp002, and
	 * B - the textual merge ends up corrupting the model with a false conflict.
	 */
	@Test
	...
	/*
	 * Try and merge logically. The merge should end without issues, without being affected
	 * by the textual conflict.
	 */
	@Test
	...
	/* 
	 * Try and merge textually. This has no relation with EMF Compare, but it does check that :
	 * A - the models were properly created by setUp003, and
	 * B - the textual merge manages to auto-merge the pseudo-conflict without corrupting the model.
	 */
	@Test
	...
	/* 
	 * Try and merge logically. The merge should end successfully, auto-merging the pseudo-conflict
	 * as needed.
	 */
	@Test
	...
	/* 
	 * Try and merge textually. This has no relation with EMF Compare, but it does check that :
	 * A - the models were properly created by setUp004, and
	 * B - the textual merge corrupts the model in which there was a conflict.
	 */
	@Test
	...
	/* 
	 * Try and merge textually. This has no relation with EMF Compare, but it does check that :
	 * A - the models were properly created by setUp005, and
	 * B - the textual merge corrupts the two models.
	 */
	@Test
	...
	/* 
	 * Try and merge textually. This has no relation with EMF Compare, but it does check that :
	 * A - the models were properly created by setUp006, and
	 * B - the textual merge ends up corrupting the models with false conflicts.
	 */
	@Test
	...
	/* 
	 * Try and merge textually. This has no relation with EMF Compare, but it does check that :
	 * A - the models were properly created by setUp007, and
	 * B - the textual merge ends up corrupting the logical model even though it says the merge ends successfully.
	 */
	@Test
	...
	/*
	 * Try and merge logically. There is a logical conflict, so we expect both files to remain untouched and the
	 * conflict marked.
	 */
	@Test
	...
	/* 
	 * Try and merge textually. This has no relation with EMF Compare, but it does check that :
	 * A - the models were properly created by setUp008, and
	 * B - the textual merge ends up corrupting the logical model even though it says the merge ends successfully.
	 */
	@Test
	...
	/* 
	 * Try and merge textually. This has no relation with EMF Compare, but it does check that :
	 * A - the models were properly created by setUp009, and
	 * B - the textual merge wouldn't have corrupted the model in this case.
	 */
	@Test
