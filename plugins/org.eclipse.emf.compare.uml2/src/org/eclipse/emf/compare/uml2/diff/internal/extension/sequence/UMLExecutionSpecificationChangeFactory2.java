 ...
 */
public class UMLExecutionSpecificationChangeFactory2 extends UMLAbstractDiffExtensionFactory {
	...
	protected EObject getDiscriminantFromDiff(Diff input) {
		...
		if (kind == DifferenceKind.ADD || kind == DifferenceKind.DELETE) {
			...
			if (container instanceof ActionExecutionSpecification) {
				result = container;
	...
	protected List<EObject> getPotentialChangedValuesFromDiscriminant(EObject discriminant) {
		...
		if (discriminant instanceof ActionExecutionSpecification) {
			result.add(((ActionExecutionSpecification)discriminant).getStart());
			result.add(((ActionExecutionSpecification)discriminant).getFinish());
			result.addAll(((ActionExecutionSpecification)discriminant).getCovereds());
		}
		return result;
	}
	...
	protected boolean isRelatedToAnExtensionAdd(ReferenceChange input) {
		return (input.getReference().isContainment() && input.getKind().equals(DifferenceKind.ADD)
				&& input.getValue() instanceof ActionExecutionSpecification
				&& ((ActionExecutionSpecification)input.getValue()).getStart() != null
				&& ((ActionExecutionSpecification)input.getValue()).getFinish() != null
				&& ((ActionExecutionSpecification)input.getValue()).getCovereds() != null
				&& !((ActionExecutionSpecification)input.getValue()).getCovereds().isEmpty() && getManagedConcreteDiscriminantKind()
