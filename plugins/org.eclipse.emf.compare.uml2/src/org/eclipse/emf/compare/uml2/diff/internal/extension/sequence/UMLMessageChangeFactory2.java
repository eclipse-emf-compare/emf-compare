 ...
 */
public class UMLMessageChangeFactory2 extends UMLAbstractDiffExtensionFactory {

	public Class<? extends UMLExtension> getExtensionKind() {
		return UMLMessageChange.class;
	}

	@Override
	protected UMLExtension createExtension() {
		return Uml2diffFactory.eINSTANCE.createUMLMessageChange();
	}
	...
	protected List<EObject> getPotentialChangedValuesFromDiscriminant(EObject discriminant) {
		...
		if (discriminant instanceof Message) {
			...
			if (recvEvent instanceof InteractionFragment && sendEvent instanceof InteractionFragment) {
	...
	protected boolean isRelatedToAnExtensionAdd(ReferenceChange input) {
		return (input.getReference().isContainment() && input.getKind().equals(DifferenceKind.ADD)
				...
				&& ((Message)input.getValue()).getReceiveEvent() != null
				&& ((Message)input.getValue()).getSendEvent() != null && getManagedConcreteDiscriminantKind()
