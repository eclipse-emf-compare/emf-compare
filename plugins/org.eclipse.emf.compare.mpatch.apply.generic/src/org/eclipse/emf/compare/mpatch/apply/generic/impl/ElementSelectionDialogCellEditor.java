...
class ElementSelectionDialogCellEditor extends DialogCellEditor {
	...
	private Object bindAddedElement(IndepAddElementChange change) {
		...
		final List<EObject> bound = mapping.getResolutionByChange().get(change).get(change.getSubModelReference());
		
		// create dialog
		final BindAddedElementDialog dialog = new BindAddedElementDialog(shell,
				mapping.getModel(), change.getSubModel(), bound);
		if (dialog.open() == Dialog.OK) {
			final List<EObject> result = (List<EObject>) dialog.getResult();
			return result;
		} else
			return null;
	}
	...
	private SymrefRefinement refineSymref(IElementReference symref) {
		...
		// create dialog
		final SymrefResolutionDialog dialog = new SymrefResolutionDialog(shell,
	...
	private final class BindAddedElementDialog extends Dialog implements ISelectionChangedListener {
		...
		private BindAddedElementDialog(Shell parent, EObject model,
				IModelDescriptor modelDescriptor, List<EObject> currentList) {
			super(parent);
		...
		protected Control createDialogArea(Composite parent) {
			...
			gd.verticalIndent = 6;
			modelTreeViewer = new TreeViewer(composite, SWT.BORDER | SWT.MULTI | SWT.FULL_SELECTION | SWT.H_SCROLL | SWT.V_SCROLL);
		...
		private void addInfoLabels(Composite contents) {
			...
			statusLabel.setLayoutData(new GridData(GridData.HORIZONTAL_ALIGN_BEGINNING, GridData.VERTICAL_ALIGN_BEGINNING, true, false));
		}
	...
	private final class SymrefResolutionDialog extends FeatureEditorDialog {
		...
		private void addEqualRefs(Composite contents) {
			...
			final List<IElementReference> equalRefs = mapping
					.getEquallyResolvingReferences((IElementReference)genericMPatchResolver.getCurrentlySelectedElement());
