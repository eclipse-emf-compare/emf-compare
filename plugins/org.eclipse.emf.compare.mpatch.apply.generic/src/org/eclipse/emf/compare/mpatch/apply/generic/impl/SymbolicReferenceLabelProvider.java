 ...
 */
public class SymbolicReferenceLabelProvider extends AdapterFactoryLabelProvider {
	 ...
	 * Default constructor.
	 * 
	 * @see SymbolicReferenceLabelProvider
	 */
	public SymbolicReferenceLabelProvider(ResolvedSymbolicReferences mapping, AdapterFactory adapterFactory) {
	...
	private Image secondColumnChangeImage(Object element, final boolean forward) {
		...
		if (mapping.getResolutionByChange().containsKey(change)) {
			...
			if (img == null) {
				if (ValidationResult.SUCCESSFUL.equals(MPatchValidator.validateElementState(change, mapping
		...
		if (mapping.getResolutionByChange().containsKey(change)) {
			...
			if (mapping.getResolutionByChange().get(change).get(ref) != null) {
				for (final EObject obj : mapping.getResolutionByChange().get(change).get(ref)) {
					final IItemLabelProvider labelProvider = (IItemLabelProvider) adapterFactory.adapt(obj,
							IItemLabelProvider.class);
					list += (list.length() == 0 ? "" : ", ") + labelProvider.getText(obj);
				}
			}
			return "[" + list + "]";
		} else
	...
	private String thirdColumnChangeText(Object element, final boolean forward) {
		if (mapping.getResolutionByChange().containsKey(element)) {
			final ValidationResult state = MPatchValidator.validateElementState((IndepChange) element, mapping
					.getResolutionByChange().get(element), false, forward);
			return MPatchValidator.VALIDATION_RESULTS.get(state);
		} else
			return "ignored";
	}
	...
	private String thirdColumnGroupText(Object element, final boolean forward) {
		...
		for (EObject change : changes) {
			if (!mapping.getResolutionByChange().containsKey(change))
				ignored++;
			else if (!ValidationResult.SUCCESSFUL.equals(MPatchValidator.validateElementState((IndepChange) change,
					mapping.getResolutionByChange().get(change), false, forward)))
				failed++;
			else
				resolved++;
		}
