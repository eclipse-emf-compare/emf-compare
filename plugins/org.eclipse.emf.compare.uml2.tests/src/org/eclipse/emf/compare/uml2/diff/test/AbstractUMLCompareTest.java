...
//		
//		URI UML2Jar = URI.createURI("jar:file:/home/mbarbero/Obeo/Projects/Ericsson/emfcompare-0/dev/target-platform/3.7RC1/plugins/org.eclipse.uml2.uml.resources_3.1.100.v201008191510.jar!/");
//		URI sysMLJar = URI.createURI("jar:file:/home/mbarbero/Obeo/Projects/Ericsson/emfcompare-0/dev/target-platform/3.7RC1/plugins/org.eclipse.papyrus.sysml_0.8.0.v201105181418.jar!/");
//		
//		uriMap.put(
...
//		uriMap.put(URI.createURI(SysmlPackage.eNS_URI), sysMLJar.appendSegment("model").appendSegment("sysml.ecore"));
	...
	protected final void testCompare(String testFolderPath) throws IOException,
	...
	private DiffModel firstNonEmptyDiffModel(DiffResourceSet diffResourceSet) {
		for (DiffModel diffModel : diffResourceSet.getDiffModels()) {
			if (diffModel.getOwnedElements().size() > 1 || (diffModel.getOwnedElements().get(0) instanceof DiffGroup && !diffModel.getOwnedElements().get(0).getSubDiffElements().isEmpty())) {
				return diffModel;
			}
		}
		return null;
	...
	private void testMerge(String testFolderPath, boolean leftToRight) throws IOException, InterruptedException {
		...
		if (leftToRight) {
			...
			originalResource = originalResourceSet.getResource(URI.createURI(".." + MODIFIED_MODEL_UML), true);
			...
			originalResource = originalResourceSet.getResource(URI.createURI(".." + ORIGINAL_MODEL_UML), true);
			mergedResource = mergeResourceSet.getResource(URI.createURI(".." + ORIGINAL_MODEL_UML), true);
		}
		
		Assert.assertEquals(ModelUtils.serialize(originalResource.getContents().get(0)), ModelUtils.serialize(mergedResource.getContents().get(0)));
	}

	private void merge(boolean leftToRight, DiffResourceSet computed_diff) {
		for (DiffModel diffModel : computed_diff.getDiffModels()) {
			for (DiffElement element : diffModel.getDifferences()) {
				if (!DiffAdapterFactory.shouldBeHidden(element)) {
	...
	private DiffResourceSet getComputedDiff(String testFolderPath)
...
//						originalResourceSet));
		...
		MatchResourceSet computed_match = MatchService.doResourceSetMatch(
				modifiedResourceSet, originalResourceSet, matchOptions);
		...
		Resource computedResource = computedResourceSet.createResource(URI
				.createURI(EXPECTED_EMFDIFF));
	...
	private ResourceSet getModelResourceSet(String testFolderPath, String model) throws IOException {
		...
		final Resource original = resourceSet.createResource(URI
				.createURI(".." + model));
		original.load(AbstractUMLCompareTest.class
				.getResourceAsStream(getDiagramKindPath() + testFolderPath
						+ model), Collections.emptyMap());
		return resourceSet;
	}

	private DiffResourceSet getExpectedDiff(
			String testFolderPath) throws IOException {
		...
		final ComparisonResourceSetSnapshot expected_diff_snapshot = (ComparisonResourceSetSnapshot) ModelUtils
				.load(AbstractUMLCompareTest.class
						.getResourceAsStream(getDiagramKindPath()
								+ testFolderPath + EXPECTED_SUFFIX),
						EXPECTED_EMFDIFF, expectedResourceSet);
