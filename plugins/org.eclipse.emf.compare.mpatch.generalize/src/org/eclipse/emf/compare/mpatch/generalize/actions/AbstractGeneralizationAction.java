...
import org.eclipse.emf.compare.mpatch.generalize.GeneralizeActivator;
...
public abstract class AbstractGeneralizationAction extends AbstractCompareAction {
	...
	protected final Status runAction(Resource input, Resource output, IProgressMonitor monitor) {
		...
		if (content instanceof MPatchModel) {
			...
			try {
				...
				if (modifications > 0) {
					...
					message = ACTION_NAME + " was not successful.";
					code = Status.WARNING;
				}
		...
		return new Status(code, GeneralizeActivator.PLUGIN_ID, message, exception);
	}
}
